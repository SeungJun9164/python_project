...
def merge_image():
    ...
    # 위의 코드와 동일 widths, heights = zip(*(x.size for x in images))
    ...
    for idx, img in enumerate(images): # enumerate : images를 열거하여 idx, img 값에 대입
        ...
        y_offest += img.size[1] # height 값만큼 더해줌
    ...
    dest_path = os.path.join(txt_dest_path.get(), "SeungJun Images.jpg") # dest_path에 txt_dest_path를 얻어와서 join
